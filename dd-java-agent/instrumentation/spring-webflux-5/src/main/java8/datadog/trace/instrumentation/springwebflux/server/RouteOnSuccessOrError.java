...
public class RouteOnSuccessOrError implements BiConsumer<HandlerFunction<?>, Throwable> {
  ...
  private static final Pattern HTTP_METHOD_REGEX = Pattern.compile("^\\([A-Z]+ &&");
  ...
  private String parseResourceName(final String routerString) {
    final String methodStripped = HTTP_METHOD_REGEX.matcher(routerString).replaceAll("");
    return SPACES_REGEX
        .matcher(SPECIAL_CHARACTERS_REGEX.matcher(methodStripped).replaceAll(""))
        .replaceAll(" ")
        .trim();
  }
}
