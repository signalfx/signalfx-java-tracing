...
class OkHttp3Test extends AgentTestRunner {
  ...
  def "sending a request creates spans and sends headers"() {
    ...
    if (agentRequest) {
      ...
      assert server.lastRequest.headers.get("x-b3-traceid") == null
      assert server.lastRequest.headers.get("x-b3-spanid") == null
    } else {
      assertTraces(1) {
        trace(0, 2) {
          span(0) {
      ...
      assert server.lastRequest.headers.get("x-b3-traceid") == new BigInteger(TEST_WRITER[0][1].traceId).toString(16).toLowerCase()
      assert server.lastRequest.headers.get("x-b3-spanid") == new BigInteger(TEST_WRITER[0][1].spanId).toString(16).toLowerCase()
    }
